/*
 * CFproxy: 通用代理服务，基于 Cloudflare Workers 实现的无服务器代理加速解决方案，支持访问被墙或受限的 URL。
 * Repo: https://github.comF55Bx9DU/CFproxy
 */

const REPO_URL = "https://github.com/F55Bx9DU/CFproxy";
    ...
    try {
        ...
        if (GITHUB_HOSTS.includes(targetUrl.hostname)) {
            ...
            if (ghToken) {
                ...
                console.log('[CFproxy] GitHub Token injected for:', targetUrl.hostname);
            } else {
                // Token 未配置或为空，记录警告（在 Workers Dashboard 日志可见）
                console.warn('[CFproxy] GH_TOKEN is not set or empty, GitHub rate limit may apply.');
            }
        ...
        <div class="input-group">
            ...
            <div class="input-wrapper">
                ...
                <div id="inputHint" class="input-hint">
                    <span>支持完整 URL 或域名 (如 github.com/F55Bx9DU)</span>
                </div>
            </div>
...
<footer>
    <p>Project <a href="${REPO_URL}" target="_blank">CFproxy</a> by <a href="https://github.com/F55Bx9DU/" target="_blank">F55Bx9DU</a></p>
...
<script>
    ...
    function checkInput() {
        ...
        if (!val) {
            ...
            setUI('reset', '<span>支持完整 URL 或域名 (如 github.com/F55Bx9DU)</span>');
            return;
        }
